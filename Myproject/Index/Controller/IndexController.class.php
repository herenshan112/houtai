...
class IndexController extends Controller {
    ...
    public function index() {
        //echo date('Y-m-d H:i:s',1480511760);
        ...
        // 新闻推荐
        $M_news = M('news');
        $index_news = $M_news->where(array('tuijian'=>1, 'pid'=>'1'))->order('id DESC')->limit(5)->select();
    ...
    public function wx_paynotify() {
        ...
        if ($notifyresult['return_code']=='SUCCESS') {
            ...
            if ($order_row['orderstatus'] == 1) {
                ...
                $ordidv=M('orders')->field('id,ordernum')->where(array('ordernum'=>$notifyresult['out_trade_no']))->find();
                $lcnr='客户：'.$user_row['username'].'&nbsp;'.$user_row['nickname'].'&nbsp;'.$user_row['phone'].'于'.date('Y-m-d H:i:s').'支付完成';
                    echo $successxmlData;
                } else {
                $ordidv=M('orders')->field('id,ordernum')->where(array('ordernum'=>$notifyresult['out_trade_no']))->find();
                $lcnr='客户：'.$user_row['username'].'&nbsp;'.$user_row['nickname'].'&nbsp;'.$user_row['phone'].'于'.date('Y-m-d H:i:s').'支付失败';
                    echo $failedxmlData;
                }
                exit;
            }
    ...
    *生成推广二维码
    ...
    public function tgqrcode(){
        ...
        $p_code = I('get.code');
        if($p_code){
            $url = WX_URL . U('Mall/index',array('code'=>$p_code));
            \QRcode::png($url, false, $errorCorrectionLevel, $matrixPointSize);
        }else{
            echo '/Public/index/img/onError.gif';
        }
    ...
    *发送验证码
    ...
    public function smscode(){
        ...
        $phone=I('post.telval')?I('post.telval'):0;
        if($phone == 0){
            echo json_encode(array('code'=>0,'msg'=>'请输入手机号码！'));
            return;
        }

        if(!preg_match("/^1[34578]{1}\d{9}$/",$phone)){  
            echo json_encode(array('code'=>0,'msg'=>'请输入正确手机号码！'));
            return;
        }
        $usm=M('user');
        if($usm->where(array('username'=>$phone))->find()){
            echo json_encode(array('code'=>0,'msg'=>'该手机号码已经被使用！请更换'));
            return;
        }
        if($usm->where(array('phone'=>$phone))->find()){
            echo json_encode(array('code'=>0,'msg'=>'该手机号码已经被使用！请更换'));
            return;
        }

        if(session('?smscodval')){
            $sesval=session('smscodval');
            if(time()-$sesval['time'] > 60){
                $codeesd=shengchancode();
                $msg="你的验证码为：".$codeesd."（5分钟内有效），请勿向他人泄漏您的验证码。【广视汇】";
                $fsjg=NewSms($phone,$msg);
                switch ($fsjg) {
                    case '000':
                        session('smscodval',array(
                            'code'          =>              $codeesd,
                            'time'          =>              time()
                        ));
                        ...
                        break;
                    case '-01':
                        echo json_encode(array('code'=>0,'msg'=>'账号余额不足'));
                        break;
                    case '-02':
                        echo json_encode(array('code'=>0,'msg'=>'未开通接口授权'));
                        break;
                    case '-03':
                        echo json_encode(array('code'=>0,'msg'=>'账号密码错误'));
                        break;
                    case '-04':
                        echo json_encode(array('code'=>0,'msg'=>'参数个数不对或者参数类型错误'));
                        break;
                    case '-110':
                        echo json_encode(array('code'=>0,'msg'=>'IP被限制'));
                        break;
                    case '-12':
                        echo json_encode(array('code'=>0,'msg'=>'其他错误'));
                        break;
                    default:
                        echo json_encode(array('code'=>0,'msg'=>'发送失败，请重试！'));
                        break;
                }
            }else{
                echo json_encode(array('code'=>0,'msg'=>'你的验证码已发送！请耐心等待！'));
                return;
            }
        }else{
            $codeesd=shengchancode();
    ...
    static function smscodfason($phone){
