...
<body>
    ...
    <div class="formbody">
		...
		<form action="/Admin/Category/cateadd" method="post" enctype="multipart/form-data" name="myform">
		<ul class="forminfo">

		<li>
			<label>所属栏目</label>      
			<select class="sellect" name="partsid">
			<?php if(is_array($parts_rows)): foreach($parts_rows as $key=>$list_item): ?><option value="<?php echo ($list_item["id"]); ?>"><?php echo ($list_item["title"]); ?></option><?php endforeach; endif; ?>
			</select>
		</li>
		
		<li><label>分类标题:</label><input name="title" class="dfinput" type="text"><i></i></li>
