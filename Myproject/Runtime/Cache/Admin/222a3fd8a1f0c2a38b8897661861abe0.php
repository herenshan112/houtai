...
<body>
    ...
    <div class="formbody">
		<div class="formtitle"><span>配送期限管理</span></div>
		<form action="/Admin/Orders/fhqxset?action=<?php echo ($action); ?>" method="post" enctype="multipart/form-data" name="myform">
		<ul class="forminfo">

		
		
		<li><label>配送期限:</label><input name="setval" class="dfinput" type="text" value="<?php echo ($tsz); ?>" onkeyup="if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\D/g,'')}" onafterpaste="if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\D/g,'')}"><i>填写整数(单位：天)</i></li>
