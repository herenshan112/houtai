...
<body>
	...
	<div class="place">
		...
		<li><a href="#">经销商<?php if(($ly) == "1"): ?>发货<?php else: ?>推广<?php endif; ?>统计</a></li>
...
.imgtable th, .imgtable td {
    ...
    <div class="rightinfo">
		<div class="tools">
			<ul class="seachform" style="float:left;">
				<form name="search_form" action="<?php echo U('lookcontjxs');?>?ly=<?php echo ($ly); ?>" method="post">
				<li><label>查询：</label><input type="text" class="scinput" name="ordernum" placeholder="请输入关键词..." value="<?php echo ($p_ordernum); ?>" />
					...
					<select name="ddcxff"  id="ddcxff" <?php if(in_array(($lookset), explode(',',"0,1"))): ?>style="display: none;"<?php endif; ?> class="scinputfb">
						<?php if(($ly) == "1"): ?><option value="2" <?php if(($ddcxff) == "2"): ?>selected<?php endif; ?>>发货</option>
						<?php else: ?>
							<option value="1" <?php if(($ddcxff) == "1"): ?>selected<?php endif; ?>>推广</option><?php endif; ?>
				...
				<li><label>&nbsp;</label><input type="button" value="导出" class="scbtn" onclick="exportongji(<?php if(($ly) == "1"): ?>3<?php else: ?>2<?php endif; ?>)" /></li>
				</form>
		...
		<table class="imgtable">
			...
			<?php if(is_array($orders_list)): foreach($orders_list as $key=>$list): ?><tr>
				...
				<td><?php
 if($list['orderstatus']>1) { echo '<p>支付方式：'; if ($list['paytype'] == 0) { echo '线下支付'; } else if ($list['paytype'] == 1) { echo '微信'; } else if ($list['paytype'] == 2) { echo '支付宝'; } echo '<p>支付时间：'; echo date('Y-m-d H:i:s', ($list['time_end'])); echo '</p>'; } else { echo '未支付'; } ?></td>
 ...
 <div class="pu_wkj" id="expordiv" style="display: none;">
	<div class="tjkj_cont">
