...
<li><a href="#">分类列表</a></li>
    ...
    <div class="rightinfo">
		...
		<table class="imgtable">
			...
			<tbody>
			<?php if($parts_rows != null): if(is_array($parts_rows)): foreach($parts_rows as $key=>$listrow): ?><tr style="height:32px">
					<td>&nbsp;</td>
					...
					<?php if(is_array($listrow['lists'])): foreach($listrow['lists'] as $key=>$sublistrow): ?><tr style="height:32px">
						<td><?php echo ($sublistrow["id"]); ?></td>
						<td>&nbsp;</td>
						<td><?php echo ($sublistrow["title"]); ?></td>
						<td><?php echo ($sublistrow["addtime"]); ?></td>
						<td><a href="<?php echo U('catemod', array('id'=>$sublistrow['id']));?>">修改</a>&#12288;<a href="<?php echo U('catedel', array('id'=>$sublistrow['id']));?>" onclick="if (confirm('确定要删除吗？')) return true; else return false;">删除</a></td>
						</tr><?php endforeach; endif; endforeach; endif; ?>
			<?php else: ?>
				<tr style="height:32px"><td colspan="9">暂无记录！</td></tr><?php endif; ?>
			</tbody>
		...
		<div class="yellow"><?php echo ($pageshow); ?></div>
