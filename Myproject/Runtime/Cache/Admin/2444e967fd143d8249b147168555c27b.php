...
    <div class="formbody">		<div class="formtitle"><span>参数设置</span></div>		<form action="/Admin/Admin/params" method="post" name="myform">		<ul class="forminfo">				<?php if(is_array($params_rows)): foreach($params_rows as $key=>$params_item): ?><li><label><?php echo ($params_item["title"]); ?>:</label><input name="<?php echo ($params_item["name"]); ?>" class="dfinput" type="text" value="<?php echo ($params_item["value"]); ?>" /><i><?php echo ($params_item["comment"]); ?></i></li><?php endforeach; endif; ?>		<li><label>&nbsp;</label><input type="submit" class="btn" value="确认修改"/></li>		</ul>		</form>    </div>
