...
<body>
    ...
    <div class="formbody">
		...
		<form action="/Admin/Products/typelist/action/<?php echo ($action); ?>/id/<?php echo ($id); ?>" method="post" enctype="multipart/form-data" name="myform">
		<ul class="forminfo">
		
		<li><label>分类标题:</label><input name="title" class="dfinput" type="text" value="<?php echo ($list["title"]); ?>"><i></i></li>
		<li><label>状态</label><input name="setval" type="radio" value="0" <?php if(($list["setval"]) != "1"): ?>checked<?php endif; ?> />关闭&nbsp;&nbsp;<input name="setval" type="radio" value="1" <?php if(($list["setval"]) == "1"): ?>checked<?php endif; ?> />启用</li>
		<li><label>排序:</label><input name="xuhao" class="dfinput" type="text" value="<?php echo ($list["xuhao"]); ?>"><i></i></li>
